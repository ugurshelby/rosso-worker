...
⚠ YALNIZ 'alltime': Ölçüm (2026-07-31) maliyetin TEK pencerede toplandığını
gösterdi — `recap_top_tracks`: hafta 6 ms, ay 55 ms, yıl 43 ms, **alltime
2211 ms**. Dar pencerelerde tarih indeksi satırları önce kesiyor; alltime'da
süzgeç düşünce 124 bin satır katalogla JOIN'lenip gruplanıyor.

Efendim'in kararı: hafta/ay/yıl RPC'de KALIR. Zaten hızlılar VE sık
değişiyorlar — paketlenselerdi kullanıcı dün dinlediğini bugün "bu hafta"da
göremezdi (yanlış olurdu, yalnız yavaş değil). Belgenin `user x dönem`
(4 satır) şeması bu yüzden gereksiz: gece yükü 4 kat olurdu, kazancın %97'si
zaten tek pencerede.

⚠ Delta YAZILMADI: top-N sıralaması ve tekil sayımlar toplanamaz
(§12.4-C'nin kendi uyarısı). Tam hesap.
...
logger = logging.getLogger("rosso.worker.period_pkg_runner")
...
#: gün atlanabilirdi. `stats_pkg_runner` ile aynı gerekçe.
STALE_AFTER_HOURS = 20


def run_period_pkg(client: Any, force: bool = False) -> dict[str, Any]:
    """Bayat paketi olan kullanıcılar için `build_user_period_pkg` çağırır.

    {outcome, users_processed, skipped, empty, errors} döner. Bir kullanıcının
    hatası diğerlerini durdurmaz (izole) — `stats_pkg_runner` ile aynı desen.
    ...
    try:
        ...
        logger.exception("period_pkg: aday sorgusu başarısız")
    ...
    for uid in user_ids:
        try:
            ...
            res = client.rpc("build_user_period_pkg", {"p_user_id": uid}).execute()
            ...
            errors += 1
            logger.warning("period_pkg başarısız user=%s: %s", uid, str(exc)[:200])
...
def _needs_refresh(client: Any, user_id: str) -> bool:
    ...
    try:
        ...
        # hesaplamak yeğdir.
        logger.warning("period_pkg: tazelik kontrolü başarısız user=%s → üretilecek", user_id)
        return True
