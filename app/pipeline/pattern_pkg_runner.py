...
⚠ YALNIZ 'alltime': Kaynak RPC'ler dönem parametresi alıyor ama Dashboard
yalnız 'alltime' çağırıyor. Dönemli çağrılar (mobil API) RPC'de KALIYOR —
paketlemek 4 dönem × kullanıcı = 4 kat gece yükü demekti (§12.3'ün
`getEraShift` dersi: sayfa ucuz yola geçer, RPC mobil için durur).

⚠ Delta YAZILMADI: Saat kutuları teknik olarak toplanabilir, ama tam hesap
168 ms. Delta altyapısı (gün bazlı ara ürün + birleştirme) bu fiyata değmez —
karmaşıklık kazançtan pahalı. Paket #4/#5'te delta gerçekten gerekli olacak.
...
logger = logging.getLogger("rosso.worker.pattern_pkg_runner")
...
#: gün atlanabilirdi. `taste_pkg_runner` ile aynı gerekçe.
STALE_AFTER_HOURS = 20


def run_pattern_pkg(client: Any, force: bool = False) -> dict[str, Any]:
    """Bayat paketi olan kullanıcılar için `build_user_pattern_pkg` çağırır.

    {outcome, users_processed, skipped, empty, errors} döner. Bir kullanıcının
    hatası diğerlerini durdurmaz (izole) — `taste_pkg_runner` ile aynı desen.
    ...
    try:
        ...
        logger.exception("pattern_pkg: aday sorgusu başarısız")
    ...
    for uid in user_ids:
        try:
            ...
            res = client.rpc("build_user_pattern_pkg", {"p_user_id": uid}).execute()
            ...
            errors += 1
            logger.warning("pattern_pkg başarısız user=%s: %s", uid, str(exc)[:200])
...
def _needs_refresh(client: Any, user_id: str) -> bool:
    ...
    try:
        ...
        # hesaplamak yeğdir.
        logger.warning("pattern_pkg: tazelik kontrolü başarısız user=%s → üretilecek", user_id)
        return True
