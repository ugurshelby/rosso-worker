"""period_pkg_runner — tazelik kuralı, boş-veri ayrımı ve izolasyon.
...
⚠ İKİNCİ KRİTİK: `test_dogru_rpc_ve_tablo_kullaniliyor`. Bu runner artık BEŞ
paket runner'ının beşincisi ve hepsi neredeyse birebir aynı. Kopyala/yapıştır
sırasında yanlış RPC/tablo adı kalırsa cron ÇALIŞIR, `success` raporlar ve
YANLIŞ paketi tazeler — hiçbir şey görünür biçimde kırılmaz.
"""
from datetime import datetime, timedelta, timezone

from app.pipeline import period_pkg_runner as runner
...
class _FakeClient:
    ...
    def rpc(self, fn, params):
        ...
        assert fn == "build_user_period_pkg", f"beklenmeyen RPC: {fn}"
...
def test_bos_kullanici_empty_doner():
    result = runner.run_period_pkg(_FakeClient([]))
    assert result["outcome"] == "empty"


def test_paketi_olmayan_kullanici_uretilir():
    client = _FakeClient(["u1"])
    result = runner.run_period_pkg(client)
...
def test_dogru_rpc_ve_tablo_kullaniliyor():
    """⚠ Beş paket runner'ı neredeyse birebir aynı.

    Yanlış RPC/tablo adı kalırsa cron ÇALIŞIR, `success` raporlar ve YANLIŞ
    paketi tazeler. Bu test o sessiz hatanın tek bekçisi.
    """
    client = _FakeClient(["u1"])
    runner.run_period_pkg(client)

    assert "build_user_period_pkg" in client.rpc_names
    ...
    assert client.tables == ["user_period_pkg"]
...
def test_force_tazelik_kontrolunu_atlar():
    ...
    result = runner.run_period_pkg(client, force=True)
...
def test_aday_sorgusu_patlarsa_error():
    ...
    result = runner.run_period_pkg(_BrokenRpc(["u1"]))
