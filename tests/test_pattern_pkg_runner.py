"""pattern_pkg_runner — tazelik kuralı, boş-veri ayrımı ve izolasyon.
...
from app.pipeline import pattern_pkg_runner as runner
...
class _FakeClient:
    ...
    def rpc(self, fn, params):
        ...
        assert fn == "build_user_pattern_pkg", f"beklenmeyen RPC: {fn}"
...
def test_bos_kullanici_empty_doner():
    result = runner.run_pattern_pkg(_FakeClient([]))
    assert result["outcome"] == "empty"


def test_paketi_olmayan_kullanici_uretilir():
    client = _FakeClient(["u1"])
    result = runner.run_pattern_pkg(client)
...
def test_force_tazelik_kontrolunu_atlar():
    ...
    result = runner.run_pattern_pkg(client, force=True)
...
def test_aday_listesi_test_profillerini_dislar():
    ...
    runner.run_pattern_pkg(_SpyClient(["u1"]))
